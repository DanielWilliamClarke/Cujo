...
export namespace Contact {
  ...
  export const render = (cv: CV, withHeader?: boolean): JSX.Element => {
    ...
    return (
      <View wrap={false} style={pdfStyles.contact}>
        ...
        {createContactItem('Phone', cv.about.entry.phone, createIconUrl('phone_in_talk'))}
        {createContactItem('Email', cv.about.entry.email, createIconUrl('email'))}
        {createContactItem('Website', cv.about.entry.website, createIconUrl('link'))}
        {githubProfile && createContactItem('Github', githubProfile.url, githubProfile.brand.iconImage?.file.url ?? createIconUrl('link'))}
        {createContactItem(
          'Address',
  ...
  const createIconUrl = (iconName: string) => `https://material-icons.github.io/material-icons-png/png/white/${iconName}/round-4x.png`
